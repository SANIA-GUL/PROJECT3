...
tic;
%% Importing Data
imageURL = 'D:\DATA\Voice images\90';
labelURL = 'D:\DATA\Voice images\ILD labels';
outputFolder = 'D:\DATA\Voice images';
imgDir = fullfile(outputFolder,'90');
...
plot(lgraph)
layer1=convolution2dLayer([3 3],64,'Stride',3,'Padding','same','Name','Encoder-Stage-1-Conv-1');
lgraph=replaceLayer(lgraph,'Encoder-Stage-1-Conv-1',layer1);
options = trainingOptions('sgdm', ... % This is the solver's name; sgdm: stochastic gradient descent with momentum
    ...
    'MaxEpochs', 3,...                 % An iteration is one step taken in the gradient descent algorithm towards minimizing the loss function using a mini batch. An epoch is the full pass of the training algorithm over the entire training set.
...
imds = imageDatastore('D:\DATA\Voice images\90', 'IncludeSubfolders', false, 'LabelSource', 'foldernames', 'FileExtensions', '.mat', 'ReadFcn', @matRead);
